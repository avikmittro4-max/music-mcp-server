...
// --- Birthday config ---
// Change BIRTHDAY_MONTH / BIRTHDAY_DAY if the date changes.
const BIRTHDAY_MONTH = 5; // May
const BIRTHDAY_DAY = 22;
const BIRTHDAY_NAME = "তুমি"; // change to the person's name if you want it personalized
const CAKE_IMAGE_URL =
  "https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=800&q=80"; // replace with your own hosted cake image if you prefer
...
function buildServer() {
  ...
  server.registerTool(
    "check_birthday",
    {
      ...
      description:
        "Checks today's date against the saved birthday. Call this once at the start of a conversation, or whenever the user greets the assistant, asks what day it is, or mentions their birthday. If today IS the birthday, this returns a warm birthday wish along with a cake image URL — speak the wish out loud AND show the image on the screen. If it is not the birthday, it returns a simple 'no special occasion today' message and nothing else should be shown or said about it.",
      inputSchema: {},
    },
    async () => {
      const now = new Date();
      ...
      if (!isBirthday) {
        return {
          content: [{ type: "text", text: "Today is not the saved birthday. No wish needed." }],
        };
      }
      ...
      return {
        content: [
          {
            type: "text",
            text: `🎂 আজ ${BIRTHDAY_NAME}-এর জন্মদিন! শুভ জন্মদিন! অনেক অনেক শুভেচ্ছা, ভালো থেকো, আনন্দে কাটুক দিনটা। 🎉🎈`,
          },
          {
            type: "image",
            data: CAKE_IMAGE_URL,
...
function connect() {
  ...
  ws.on("open", async () => {
    console.log("Connected to XiaoZhi. Registering tools...");
    const server = buildServer();
    const transport = new WebSocketBridgeTransport(ws);
    await server.connect(transport);
    console.log("MCP tools are now live on the XiaoZhi agent.");
  });
